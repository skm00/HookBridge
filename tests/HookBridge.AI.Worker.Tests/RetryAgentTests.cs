...
public sealed class RetryAgentTests
{
    ...
    [Fact]
    public async Task DelayCalculation_JitterDoesNotExceedMaxDelayCap()
    {
        var agent = CreateAgent(new RetryAgentOptions { BaseDelaySeconds = 30, MaxDelaySeconds = 100, EnableJitter = true, JitterPercentage = 10 });
        var response = await agent.AnalyzeAsync(CreateRequest(429, retryCount: 2));
        response.RetryDelaySeconds.Should().Be(100);
    }
