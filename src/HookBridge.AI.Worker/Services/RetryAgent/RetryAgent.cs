...
public sealed class RetryAgent : IRetryAgent
{
    ...
    public int CalculateDelaySeconds(RetryAgentDecision decision, int retryCount)
    {
        ...
        var maxDelaySeconds = Math.Max(0, _options.MaxDelaySeconds);
        var capped = (int)Math.Min(Math.Max(0, computed), maxDelaySeconds);
        return Math.Min(ApplyJitter(capped), maxDelaySeconds);
    }
