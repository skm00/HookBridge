...
public sealed class WebhookFailureAnomalyDetectionService : IWebhookFailureAnomalyDetectionService
{
    ...
    public WebhookFailureAnomalyDetectionResponseDto DetectAnomalies(WebhookFailureAnomalyDetectionRequestDto request, DateTime calculatedAtUtc)
    {
        ...
        AddPercentageAnomaly(anomalies, "FailureRate", FailureRate(current), FailureRate(baseline), 50, 18, "Webhook delivery failure rate increased significantly.", "Inspect receiver availability, recent deployments, and webhook delivery error patterns.");
        ...
        AddPercentageAnomaly(anomalies, nameof(current.TimeoutCount), current.TimeoutCount, baseline.TimeoutCount, 50, 12, "Timeout failures increased significantly.", "Check receiver availability and network latency.");
        ...
        AddPercentageAnomaly(anomalies, nameof(current.AuthenticationFailureCount), current.AuthenticationFailureCount, baseline.AuthenticationFailureCount, 25, 12, "Authentication failures increased significantly.", "Verify credentials, token expiry, and header configuration.");
