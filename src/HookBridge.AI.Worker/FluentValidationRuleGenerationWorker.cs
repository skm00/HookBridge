...
public sealed class FluentValidationRuleGenerationWorker : BackgroundService
{
    ...
    public FluentValidationRuleGenerationWorker(
        IFluentValidationRuleGenerationConsumer consumer,
        IFluentValidationRuleGenerationAgent agent,
        IFluentValidationRuleGenerationRepository repository,
        ILogger<FluentValidationRuleGenerationWorker> logger,
    ...
    {
        if (string.IsNullOrWhiteSpace(_kafkaOptions.ValidationRuleGenerationTopic))
        {
        ...
        await foreach (var message in _consumer.ConsumeAsync(stoppingToken))
        {
            ...
            var result = FluentValidationRuleGenerationResult.FromResponse(response, request);
