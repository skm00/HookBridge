...
public sealed class SecurityAgentRequestDto : IValidatableObject
{
    [Required]
    ...
    public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        if (!string.IsNullOrWhiteSpace(TargetUrl) && !Uri.TryCreate(TargetUrl, UriKind.Absolute, out _)) yield return new ValidationResult("TargetUrl must be a valid absolute URL when provided.", [nameof(TargetUrl)]);
    }
}
